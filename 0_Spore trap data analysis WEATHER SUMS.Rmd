...
Decatur_21 <- read.csv(file.path(path, 'Decatur 2021 - Hourly.csv'), 
                       skip = 5, col.names = col_names) %>%
  na.trim() %>%
  mutate(Location = 'Decatur', .after = 'Date')

Decatur_22 <- read.csv(file.path(path, 'Decatur 2022 - Hourly.csv'), 
                       skip = 5, col.names = col_names) %>%
  na.trim() %>%
  mutate(Location = 'Decatur', .after = 'Date')

Decatur_23 <- read.csv(file.path(path, 'Decatur 2023 - Hourly.csv'), 
                       skip = 5, col.names = col_names) %>%
  na.trim() %>%
  mutate(Location = 'Decatur', .after = 'Date')
...
EastLansing_21 <- read.csv(file.path(path, 'East Lansing 2021 - Hourly.csv'), 
                           skip = 5, col.names = col_names) %>%
  na.trim() %>%
  mutate(Location = 'East Lansing', .after = 'Date')

EastLansing_22 <- read.csv(file.path(path, 'East Lansing 2022 - Hourly.csv'), 
                           skip = 5, col.names = col_names) %>%
  na.trim() %>%
  mutate(Location = 'East Lansing', .after = 'Date')

EastLansing_23 <- read.csv(file.path(path, 'East Lansing 2023 - Hourly.csv'), 
                           skip = 5, col.names = col_names) %>%
  na.trim() %>%
  mutate(Location = 'East Lansing', .after = 'Date')
```
